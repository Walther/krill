...
fn model(app: &App) -> Model {
    ...
    // We now create a buffer that will store our vertices
    let vertex_buffer = {
    ...
    let vert_path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/shaders/vertex.glsl");
    let frag_path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/shaders/fragment.glsl");
    // Create a watcher that will reload both shaders if there is any change to the
    // parent directory eg. '/shaders/'
    let shade_watcher =
...
fn view(app: &App, model: &Model, frame: Frame) -> Frame {
    ...
    // Set up uniforms
    graphics.uniform_buffer.next({iTime: app.time}).unwrap();
